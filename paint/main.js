...
brushSlider.addEventListener('change', (event) => {
  ...
  brushSize.textContent = event.target.value;
  displayBrushSize();
});
...
saveStorageBtn.addEventListener('click', () => {
  localStorage.setItem('draw', JSON.stringify(drawnArray));
//   // Active Tool
...
loadStorageBtn.addEventListener('click', () => {
  if (localStorage.getItem('draw')) {
    drawnArray = JSON.parse(localStorage.getItem('draw'));
    restoreCanvas();
  // Active Tool
    activeToolEl.textContent = 'Canvas Loaded';
    setTimeout(switchToBrush, 1500);
  } else{
    activeToolEl.textContent = 'No saved images';
    setTimeout(switchToBrush, 1500);
  }
});
